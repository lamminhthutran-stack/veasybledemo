...
function ExecutorTasks() {
  ...
  const visibleTasks = availableTasks.filter(
    (t) => !declinedIds.includes(t.id) && !cancelledIds.includes(t.id) && t.status !== "accepted",
  );
    ...
    <div className="min-h-screen bg-[#F7F8FA] flex flex-col pb-20">
      ...
      <div className="bg-white px-4 pt-12 pb-4 shadow-sm border-b border-gray-100">
        ...
        <label className="mt-3 flex items-center justify-between bg-gray-50 p-3 rounded-[5px] border border-gray-100 cursor-pointer">
          ...
          <div
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${filterByAvailability ? "bg-[#F97316]" : "bg-gray-200"}`}
          >
            <input
              type="checkbox"
              className="sr-only"
            ...
            />
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${filterByAvailability ? "translate-x-6" : "translate-x-1"}`}
            />
          </div>
        </label>
      </div>
      ...
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-2">
        <p className="text-xs text-gray-500 flex-1">{displayedTasks.length} task(s)</p>
        <button
      ...
      {filterByAvailability && hasNoAvailability ? (
        <div className="mx-4 mt-4 bg-orange-50 rounded-[5px] p-4 border border-orange-100 text-center">
          ...
          <Link
            to="/executor/profile"
            className="inline-block bg-[#F97316] text-white text-xs font-semibold px-4 py-2 rounded-[5px]"
          >
            Go to Settings
          </Link>
        </div>
...
function BrowseTaskCard({
  ...
  return (
    <div
      className={`bg-white rounded-[5px] p-4 shadow-sm border border-gray-100 ${declined ? "opacity-60" : ""}`}
    >
      <Link
        to="/executor/task/$id"
        params={{ id: task.id }}
