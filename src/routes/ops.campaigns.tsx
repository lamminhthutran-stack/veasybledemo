    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {["All Cities", "All Statuses"].map((f) => (
          <button
            key={f}
            className="text-xs px-3 py-1.5 rounded-[5px] border border-border bg-card"
          >
            {f}
          </button>
        ))}
      ...
      <div className="bg-card border border-border rounded-[5px] overflow-hidden">
        ...
        {filteredCampaigns.length === 0 ? (
          <div className="p-8 text-center text-sm text-gray-500">
            No campaigns match the selected date filter.
          </div>
        ) : (
            ...
            return (
              <div key={c.id} className="border-b border-border last:border-0">
                ...
                >
                  ...
                  <ChevronDown
                    className={`w-4 h-4 transition ${open === c.id ? "rotate-180" : ""}`}
                  />
                </button>
                {open === c.id && (
                  <div className="bg-surface px-6 py-4 space-y-2">
                    {[
                      ...
                      <div
                        key={i}
                        className="flex items-center justify-between text-sm bg-card border border-border rounded-md px-3 py-2"
                      >
                        <div>
