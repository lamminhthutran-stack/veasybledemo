...
function Submitted() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="text-2xl font-bold mt-6">{t("pop_submitted_awaiting_review")}</h1>
      <p className="text-sm text-muted-foreground mt-2 max-w-xs">
        {t("task_marked_complete") ?? "Your task is now under review."}
      </p>

      <div className="bg-card border border-border rounded-[5px] p-4 mt-6 w-full text-left">
        <div className="text-xs uppercase tracking-wide text-muted-foreground mb-1">{t("task_summary")}</div>
        <div className="font-semibold text-sm">{tsk.campaign}</div>
        <div className="text-xs text-muted-foreground">{tsk.store}</div>
        <div className="text-xs text-muted-foreground mt-1">{t("completed_at")} {ts}</div>
      </div>
