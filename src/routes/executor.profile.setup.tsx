    ...
    <div className="flex flex-col min-h-screen">
      <header className="sticky top-0 z-10 bg-navy text-navy-foreground px-4 py-3 flex items-center gap-3">
        <button
          ...
          className="opacity-80 hover:opacity-100 min-h-[44px] flex items-center"
        >
      ...
      <main className="flex-1 p-5 space-y-5">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {"Just one more step to start accepting tasks!"}
          </p>
        </div>
        ...
        <Section title={"Personal Info"}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <button className="text-xs px-3 py-1.5 border border-border rounded-md min-h-[44px]">
              {"Upload photo"}
            </button>
          </div>
...
function Step({ label, done, active }: { label: string; done?: boolean; active?: boolean }) {
  return (
    <div className="flex items-center gap-1">
      <span
        className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${done ? "bg-success text-white" : active ? "bg-orange text-orange-foreground" : "bg-surface text-muted-foreground border border-border"}`}
      >
