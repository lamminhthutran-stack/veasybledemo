      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-border rounded-md bg-card text-sm"
          />
        </div>
        {["City", "Tier", "Rating"].map((f) => (
          <button key={f} className="text-sm px-3 py-2 border border-border rounded-[5px] bg-card">
            {f}
          </button>
        ))}
      </div>
          ...
          >
            ...
            <div>
              {u.status === "Dormant" ? (
                ...
                <span
                  className={`text-[10px] font-semibold px-2 py-1 rounded-full ${getRatingStatus(u.rating).color}`}
                >
