    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[5px] shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
          ...
          <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-[5px] transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <section>
                <h3 className="text-sm font-bold uppercase tracking-wide text-gray-900 mb-3 border-b pb-2">Brand Requirements</h3>
              ...
              <section>
                <h3 className="text-sm font-bold uppercase tracking-wide text-gray-900 mb-3 border-b pb-2">Required Materials</h3>
                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                  {richTask?.printStation?.materials 
                    ? richTask.printStation.materials.split(',').map((m, i) => <li key={i}>{m.trim()}</li>)
                    : <li>No specific materials required.</li>}
                </ul>
              </section>

              <section>
                <h3 className="text-sm font-bold uppercase tracking-wide text-gray-900 mb-3 border-b pb-2">SOP Instructions</h3>
                <div className="space-y-2">
                  {richTask?.sopItems ? richTask.sopItems.map((sop, i) => (
                    <div key={i} className="flex gap-2 text-sm text-gray-700">
                      <span className="text-gray-400">{i + 1}.</span>
                      <span>{sop}</span>
                    </div>
                  )) : (
                    <p className="text-sm text-gray-500 italic">Follow standard execution guidelines.</p>
                  )}
                </div>
              </section>
            </div>
            ...
            <div className="space-y-6">
              <section>
                <h3 className="text-sm font-bold uppercase tracking-wide text-gray-900 mb-3 border-b pb-2">Proof of Performance</h3>
                ...
                <div className="mb-4 text-xs font-medium bg-gray-50 border rounded-[5px] px-3 py-2 flex items-center justify-between">
                  ...
                  {pickupTime ? (
                    <span className="text-green-600 font-semibold">{new Date(pickupTime).toLocaleString()}</span>
                  ) : (
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="aspect-video bg-gray-100 rounded-[5px] border border-gray-200 flex flex-col items-center justify-center text-gray-400 hover:bg-gray-200 cursor-pointer transition-colors group">
                ...
                <div className="bg-gray-50 border border-gray-100 rounded-[5px] p-4 mb-4">
                  <h4 className="text-xs font-semibold text-gray-900 uppercase mb-2">Checklist Completed</h4>
